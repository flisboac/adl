...
int_literal static_data::name {1};
...
bool is_valid(detection_state a) { return a == detection_state::valid; }
bool is_invalid(detection_state a) { return a == detection_state::invalid; }
bool is_undefined(detection_state a) { return a == detection_state::undefined; }
char const* name_of(detection_state a) {
    switch(a) {
...
namespace common {
    ...
    detection_state has_traits(typename VT::enclosing_type*) {
        if (!D) return detection_state::invalid;
        return detection_state::valid;
    }
    template <typename E, typename F, typename VT, bool D = VT::detected>
    detection_state has_traits(typename VT::fallback_type*) {
        if (!D) return detection_state::invalid;
        return detection_state::valid;
    }
    template <typename E, typename F, typename VT, bool D = VT::detected>
    detection_state has_traits(VT*) {
        if (D) return detection_state::invalid;
        return detection_state::valid;
    }

    template <typename E, typename F, typename VT>
    detection_state has_enclosing_type(typename VT::enclosing_type* dummy) {
    ...
    template <typename E, typename F, typename VT>
    detection_state has_enclosing_type(E* dummy) { return detection_state::undefined; }


    template <typename E, typename F, typename VT>
    detection_state has_fallback_type(typename VT::fallback_type* dummy) {
    ...
    template <typename E, typename F, typename VT>
    detection_state has_fallback_type(E* dummy) { return detection_state::undefined; }


    template <typename E, typename F, typename VT, bool FB = VT::is_using_fallback>
    detection_state has_is_using_fallback(typename VT::fallback_type* dummy) {
    ...
    template <typename E, typename F, typename VT, bool FB = VT::is_using_fallback>
    detection_state has_is_using_fallback(typename VT::enclosing_type* dummy) {
        if (FB) return detection_state::invalid;
        if (has_fallback_type<E, F, VT>(nullptr) != detection_state::undefined) return detection_state::invalid;
        return detection_state::valid;
    }
    template <typename E, typename F, typename VT>
    detection_state has_is_using_fallback(E* dummy) { return detection_state::undefined; }


    template <typename E, typename F, typename VT, typename test_type = typename VT::selected_type>
    detection_state has_selected_type(typename VT::enclosing_type* dummy) {
    ...
    template <typename E, typename F, typename VT, typename test_type = typename VT::selected_type>
    detection_state has_selected_type(typename VT::fallback_type* dummy) {
        using original_type = typename VT::fallback_type;
        if (!std::is_same<test_type, original_type>::value) return detection_state::invalid;
        return detection_state::valid;
    }
    template <typename E, typename F, typename VT>
    detection_state has_selected_type(E* dummy) { return detection_state::undefined; }


    template <typename E, typename F, typename VT, typename test_type = typename VT::derived_type>
    detection_state has_derived_type(typename VT::enclosing_type* dummy) {
    ...
    template <typename E, typename F, typename VT, typename test_type = typename VT::derived_type>
    detection_state has_derived_type(typename VT::fallback_type* dummy) {
        if (std::is_same<test_type, void>::value) return detection_state::invalid;
        if (std::is_same<test_type, E>::value) return detection_state::invalid;
        if (std::is_same<test_type, F>::value) return detection_state::invalid;
        if (!std::is_base_of<E, test_type>::value) return detection_state::invalid;
    ...
    template <typename E, typename F, typename VT>
    detection_state has_derived_type(E* dummy) { return detection_state::undefined; }


    template <typename E, typename F, typename VT, typename test_type = typename VT::extension_type>
    detection_state has_extension_type(typename VT::enclosing_type* dummy) {
        if (std::is_same<test_type, void>::value) return detection_state::invalid;
        if (std::is_same<test_type, E>::value) return detection_state::invalid;
        if (std::is_same<test_type, F>::value) return detection_state::invalid;
        if (std::is_base_of<E, test_type>::value) return detection_state::invalid;
    ...
    template <typename E, typename F, typename VT, typename test_type = typename VT::extension_type>
    detection_state has_extension_type(typename VT::fallback_type* dummy) {
        if (std::is_same<test_type, void>::value) return detection_state::invalid;
    ...
    template <typename E, typename F, typename VT>
    detection_state has_extension_type(E* dummy) { return detection_state::undefined; }


    template <typename VT>
    typename VT::functor_type get_functor(typename VT::functor_type* dummy) {
        return typename VT::functor_type();
    };
    template <typename VT>
    dummy_functor_type get_functor(VT* dummy) { return dummy_functor_type(); };
...
detection_state has_traits(decltype(VT::detected)*) {
...
detection_state has_is_using_fallback(typename V<E, F>::selected_type* dummy) {
...
detection_state has_selected_type(typename VT::selected_type* dummy) {
    if (!std::is_same<test_type, ST>::value) return detection_state::invalid;
...
detection_state has_derived_type(typename VT::derived_type* dummy) {
    if (!std::is_same<test_type, ST>::value) return detection_state::invalid;
...
detection_state has_extension_type(typename VT::extension_type* dummy) {
    if (!std::is_same<test_type, ST>::value) return detection_state::invalid;
...
template <typename E, typename F, template <typename, typename> class V, template <typename, typename> class S, typename VT = V<E, F>, typename ST = S<E, F>>
detection_state has_functor_type(typename VT::functor_type* dummy) {
    if (!std::is_same<typename VT::functor_type, ST>::value) return detection_state::invalid;
    return detection_state::valid;
}
template <typename E, typename F, template <typename, typename> class V, template <typename, typename> class S>
detection_state has_functor_type(E* dummy) { return detection_state::undefined; }


template <typename E, typename F, template <typename, typename> class V, typename A>
detection_state test_functor(typename V<E, F>::functor_type* dummy, bool (*test_fn)(decltype(std::declval<typename V<E, F>::functor_type>()(std::declval<std::initializer_list<A>>()))), std::initializer_list<A> list) {
...
detection_state test_functor(typename V<E, F>::functor_type* dummy, bool (*test_fn)(decltype(std::declval<typename V<E, F>::functor_type>()(std::declval<Args>()...))), Args... args) {
    auto result = test_fn( typename V<E, F>::functor_type()(std::forward<Args>(args)...) );
    return result ? detection_state::valid : detection_state::invalid;
};
template <typename E, typename F, template <typename, typename> class V>
detection_state test_functor(E* dummy, ...) {
